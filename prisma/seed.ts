...
async function main() {
  ...
  const startupXYZ = await prisma.company.create({
    data: {
      ...
      mode: CompanyMode.PRE_ATS,
      plan: PlanType.PRE_ATS_SCREENING,
    },
  });
  ...
  // Create pipeline stages for Full ATS jobs
  ...
  const candidates = await Promise.all([
    // Candidate 1 - Strong match for Senior Backend
    ...
    // Candidate 2 - Good match for Junior Frontend
    prisma.candidate.create({
      data: {
    ...
    // Candidate 5 - Unassigned candidate
    prisma.candidate.create({
      data: {
  ...
  // Create candidate scores for assigned candidates
  await Promise.all([
  ...
  // Create candidate stage assignments
  await Promise.all([
    prisma.candidateStage.create({
      data: {
        candidateId: candidates[0].id,
        stageId: seniorDevStages[1].id, // Screening
      },
    }),
    prisma.candidateStage.create({
      data: {
        candidateId: candidates[1].id,
        stageId: juniorDevStages[0].id, // New
      },
    }),
    prisma.candidateStage.create({
      data: {
        candidateId: candidates[2].id,
        stageId: seniorDevStages[0].id, // New
      },
    }),
  ]);
...
The {{company_name}} Hiring Team`,
  ...
  // Create sample candidate actions
  await Promise.all([
